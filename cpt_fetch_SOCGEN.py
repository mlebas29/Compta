#!/usr/bin/env python3
"""
cpt_fetch_SOCGEN.py - Récupération automatique des relevés Société Générale
...
Usage:
  ./cpt_fetch_SOCGEN.py           # Mode normal
  ./cpt_fetch_SOCGEN.py -v        # Mode verbeux

Fichiers générés (selon les comptes configurés) :
  - {numéro_compte}.csv                    # Opérations compte courant
  - Export_{numéro}_*.csv                  # Opérations comptes épargne
  - SG_{file_key}_supports.xlsx            # Positions assurance vie
  - SG_{file_key}_operations.pdf           # Opérations assurance vie
...
try:
    ...
    OCR_AVAILABLE = ensure_tesseract_cmd()
...
Comptes multiples. Champs techniques par compte (onglet Comptes) :

- Type SG : principal, epargne ou assurance_vie (un seul compte principal
  est pris en compte).
- Numéro : numéro de compte (principal et épargne).
- ID technique : identifiant interne SG, propre à chaque compte. Le relever
  dans la barre d'adresse, en naviguant sur le compte concerné, puis le
  recopier tel quel (garder les caractères encodés %2F, %2B, %3D…) :
    . épargne        valeur après "prestationIdTechnique=" dans
                     .../cbo/index-react-authsec.html#/operations?...
    . assurance vie  dernier segment de
                     .../avd/index-authsec.html#/accueil_contrat/<id>
- Clé fichiers : assurance vie uniquement, sert au nommage des fichiers.
...
COMPTES_EPARGNE = [
    ...
    for a in _sg_accounts
    if a.get('type_sg') == 'epargne'
]

# Assurances vie — (file_key, name, id_technique)
ASSURANCES_VIE = [
    (a['file_key'], a['name'], a['id_technique'])
    for a in _sg_accounts
    if a.get('type_sg') == 'assurance_vie'
]
...
class SgFetcher(BaseFetcher):
    ...
    def _click_virtual_keyboard(self, password):
        ...
        if not OCR_AVAILABLE:
            raise Exception(tesseract_install_hint())
        ...
        digit_buttons = {}
        attempted_ids = []  # spans avec bbox valide (candidats à la déduction)
        ...
        # Fallback : si 9/10 mappés, le chiffre manquant est forcément sur l'un
        # des spans non mappés. La grille SG est 4×4 (16 spans) dont 10 chiffres
        # et 6 décoys (cases vides). Le vrai chiffre se distingue des décoys par
        # densité de pixels — décoys ~vides, chiffre = forme dessinée.
        if len(digit_buttons) == 9:
            mapped_ids = set(digit_buttons.values())
            unmapped = [sid for sid in attempted_ids if sid not in mapped_ids]
            missing = next(d for d in '0123456789' if d not in digit_buttons)

            # Compter pixels sombres dans chaque span non mappé
            id_to_span = {s.get_attribute('id'): s for s in key_spans}
            candidates = []  # (sid, dark_px_count)
            for sid in unmapped:
                span = id_to_span.get(sid)
                if not span:
                    continue
                box = span.bounding_box()
                if not box:
                ...
                dark = sum(1 for p in crop.getdata() if p < 100)
                candidates.append((sid, dark))

            candidates.sort(key=lambda t: t[1], reverse=True)
            self.logger.debug(
                f"Densité spans non mappés: "
                f"{', '.join(f'{s}={d}px' for s, d in candidates)}"
            )

            # Le plus dense parmi les unmapped = chiffre manqué par OCR.
            # Seuil de sécurité : au moins 2× plus dense que le 2e candidat
            # (sinon ambiguïté → on ne devine pas, plus sûr d'échouer).
            if len(candidates) >= 2 and candidates[0][1] >= 2 * candidates[1][1] \
                    and candidates[0][1] > 20:
                winner_id, winner_px = candidates[0]
                digit_buttons[missing] = winner_id
                self.logger.info(
                    f"Chiffre '{missing}' déduit par densité "
                    f"({winner_id}, {winner_px}px vs 2e {candidates[1][1]}px)"
                )
            else:
                top = candidates[0] if candidates else ('?', 0)
                self.logger.warning(
                    f"Déduction impossible pour '{missing}': "
                    f"top={top}, candidats={candidates[:3]}"
                )
    ...
    def export_assurance_vie_supports(self, assurance_info):
        ...
        Args:
            assurance_info: tuple (file_key, name, id_technique)

        Returns:
            Path du fichier téléchargé ou None
        """
        file_key, nom, id_technique = assurance_info
        ...
        try:
            ...
            try:
                ...
                download = download_info.value
                new_name = f"SG_{file_key}_supports.xlsx"
    ...
    def print_assurance_vie_operations_pdf(self, assurance_info):
        ...
        file_key, nom, id_technique = assurance_info
        url = f"{URL_BASE_AVD}#/accueil_contrat/{id_technique}"
        output_filename = f"SG_{file_key}_operations.pdf"
    ...
    def run(self):
        ...
        if len(epargne) < len(COMPTES_EPARGNE):
