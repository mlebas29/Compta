...
def setup_sandbox(scenario_dir, extra_copies=None, code_root=None):
    ...
    # Copies configs depuis les fixtures TNR livrées (anonymisées) — jamais depuis l'instance PROD,
    # pour que le scénario soit auto-suffisant et déterministe (cf. tests/tnr/fixtures/).
    fixtures_dir = scenario_dir.parent / 'fixtures'
    for cfg in fixtures_dir.glob('config*.json'):
        shutil.copy2(cfg, sandbox / cfg.name)
    fixture_ini = fixtures_dir / 'config.ini'
    if fixture_ini.exists():
        shutil.copy2(fixture_ini, sandbox / 'config.ini')
