...
# tkinter optionnel : voir note dans gui_devises.py — HeadlessGUI n'appelle
# pas les méthodes UI, seulement les méthodes UNO/openpyxl du mixin.
try:
...
class AccountsMixin:
        ...
        def on_ok():
            ...
            def worker():
                if HAS_UNO:
                    ...
                    # Sur daemon, extraire les kwargs depuis new_acct.
                    # Les flags GUI-internes (_is_new, row) ne traversent pas.
                    self._daemon_call(
                        'add_account',
                        intitule=intitule,
                        acct_type=new_acct['type'],
                        devise=new_acct['devise'],
                        sous_type=new_acct['sous_type'],
                        domiciliation=new_acct['domiciliation'],
                        titulaire=new_acct['titulaire'],
                        propriete=new_acct['propriete'],
                        site=new_acct['site'],
                        date_debut=None,  # solde_init est dans montant_debut
                        montant_debut=new_acct.get('montant_debut'),
                        equiv_euro_debut=new_acct.get('equiv_euro_debut'),
                        controle=ctrl_var.get(),
                    )
    ...
    def _bien_add(self):
        ...
        # Devises cotées (feuille Cotations) — EUR par défaut, fallback si vide
        cotation_devises = sorted(self.cotations_meta.keys()) or ['EUR']
        ...
        for label, key, widget_type, values, default in [
            ...
            ('Devise :', 'devise', 'combo', cotation_devises, 'EUR'),
    ...
    def _acct_edit(self):
        ...
        def on_ok():
            ...
            if renamed:
                def worker_rename():
                    if HAS_UNO:
                        self._rename_account(old_intitule, new_intitule)
                    else:
                        self._daemon_call('rename_account',
                                          old_intitule=old_intitule,
                                          new_intitule=new_intitule)
                self._run_uno_operation(
                    'Renommage du compte',
                    worker_rename,
                ...
                modify_fields = {
                    ...
                    'sous_type': avoirs_acct['sous_type'],
                    'domiciliation': avoirs_acct['domiciliation'],
                    'titulaire': avoirs_acct['titulaire'],
                    'propriete': avoirs_acct['propriete'],
                }
                ...
                def worker_modify():
                    if HAS_UNO:
                        ...
                        self._daemon_call('modify_account',
    ...
    def _rename_account(self, old_name, new_name, doc=None):
    ...
    def _acct_delete(self):
        ...
        def worker():
            if HAS_UNO:
                ...
                self._daemon_call('delete_account', intitule=label)
    ...
    def _acct_purge(self):
        ...
        def worker():
            if HAS_UNO:
                self._purge_account_uno(label, titles)
            else:
                self._daemon_call('purge_account', intitule=label)
    ...
    def _purge_account_uno(self, account_name, titles, doc=None):
    ...
    def _pv_title_rename(self):
        ...
        def on_ok():
            ...
            def worker():
                if HAS_UNO:
                    self._rename_pv_title(account_name, old_name, new_name, pv_row)
                else:
                    self._daemon_call('rename_pv_title',
                                      account_name=account_name,
                                      old_title=old_name, new_title=new_name,
                                      pv_row=pv_row)
    ...
    def _pv_title_delete(self):
        ...
        def worker():
            if HAS_UNO:
                self._delete_pv_title(account_name, title_name, pv_row)
            else:
                self._daemon_call('delete_pv_title',
                                  account_name=account_name,
                                  title_name=title_name, pv_row=pv_row)
    ...
    def _rename_pv_title(self, account_name, old_name, new_name, pv_row, doc=None):
    ...
    def _delete_pv_title(self, account_name, title_name, pv_row, doc=None):
    ...
    def _run_uno_operation(self, label, worker_fn, on_success=None):
        ...
        """
        # Les gardes lock/port ne s'appliquent qu'au chemin in-process UNO
        # (Linux). Sur Mac (HAS_UNO=False) le worker passe par le daemon qui
        # gère son propre soffice — port 2002 et lock file sont normalement
        # occupés par lui pendant toute la session GUI.
        from inc_uno import HAS_UNO
        if HAS_UNO:
    ...
    def _run_title_save(self, account_name, title_name, devise, date_init):
        ...
        def worker():
            if HAS_UNO:
                self._insert_pv_title(account_name, title_name, devise, date_init)
            else:
                # daemon.add_title sérialise date_init en str via JSON.
                date_str = date_init.strftime('%d/%m/%Y') if date_init else None
                self._daemon_call('add_title',
                                  account_name=account_name,
                                  title_name=title_name, devise=devise,
                                  date_init=date_str)
