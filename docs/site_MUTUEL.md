...
- **Jointure par RIB, pas par nom** : la clé est le **dernier bloc du RIB** (numéro de compte), présent à la fois dans la synthèse et dans le nom d'onglet. Indispensable car des homonymes (plusieurs prêts de même intitulé) rendent le « par nom » ambigu. Chaque compte déclare son RIB (`config_accounts.json[MUTUEL]`, champ saisi à la création dans l'onglet Avoirs).
