...
- Le `#Solde` de chaque compte vient de `wise_balances.csv` (fetch). Le **solde EUR se lit sur la page du groupe multi-devises**, pas sur `/home` (qui y affiche l'EUR comme devise de base, d'un montant sans rapport avec le vrai solde-devise).
- **Intérêts Wise Assets suivis (v5.21.0)** : le rendement d'un solde rémunéré est **invisible aux transactions** (crédité au solde) → collecté à part (`wise_interest.csv`, « Rendements depuis le début ») et booké à l'import par un **delta C-déduit**. Le contrôle de cohérence **lit le classeur** : `Σ(ops « Intérêts » du compte cash)` doit égaler le cumul scrapé `C`, sinon le `#Solde` est retenu → « ⚠ Solde calculé » (virement manquant ?). ⚠ **Invariant** : sur un compte cash Wise, **seule l'op d'accrual bookée automatiquement (`Intérêts Wise`) doit porter la catégorie « Intérêts »** — tout autre crédit qui y atterrirait (règle de catégorisation erronée, type CSV ignoré) casserait ce contrôle. Détail : docstrings `cpt_format_WISE.build_soldes_and_interest` / `cpt_fetch_WISE.fetch_interest`.
