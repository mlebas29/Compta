...
class EtoroFetcher(BaseFetcher):
    ...
    _FB_HOSTS = ("facebook.com", "fbcdn.net", "connect.facebook.net")

    def launch_browser(self):
        """Override : bloque les requêtes Facebook au niveau réseau.

        eToro embarque un Facebook Pixel qui déclenche un popup de
        consentement cookies Facebook au-dessus de la page login,
        masquant le formulaire email/password.
        """
        super().launch_browser()
        self.context.route("**/*", self._block_facebook)

    def _block_facebook(self, route):
        if any(host in route.request.url for host in self._FB_HOSTS):
            route.abort()
        else:
            route.continue_()
