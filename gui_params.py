...
class ParamsMixin:
    ...
    def _build_tab_params(self, tab=None):
        ...
        # Secrets — vue GLOBALE de la table chiffrée (créer/supprimer, voir les
        # orphelins). La contrepartie contextuelle — la réf d'UN site — vit dans
        # l'onglet Sites, sans liste ni colonne « Utilisé par » puisque le site y
        # est implicite.
        idf = ttk.LabelFrame(scroll_frame, text='Table chiffrée (GPG)',
                             padding=10)
        idf.pack(fill='x', padx=10, pady=5)
        # Symétrique du cadre « Authentification » de l'onglet Sites : chaque porte
        # pointe vers l'autre bout du pont que l'utilisateur établit à la main.
        ttk.Label(idf, text='La 1ʳᵉ colonne (Réf) doit trouver sa correspondance dans '
                            'le cadre Authentification d\'un site — onglet Sites.',
                  style='Hint.TLabel', wraplength=560,
                  justify='left').pack(anchor='w', pady=(0, 6))
        ttk.Button(idf, text='Éditer',
