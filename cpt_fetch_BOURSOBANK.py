#!/usr/bin/env python3
"""
cpt_fetch_BOURSOBANK.py - Récupération automatique des données BoursoBank
...
- tesseract installé (Linux: apt install tesseract-ocr ;
                     Mac Ventura: port install tesseract ;
                     Mac Sonoma+: brew install tesseract)

Usage:
  ./cpt_fetch_BOURSOBANK.py           # Mode normal
  ./cpt_fetch_BOURSOBANK.py -v        # Mode verbeux

Fichiers générés:
  - dropbox/BOURSOBANK/export_compte_principal.csv (opérations compte chèque)
  - dropbox/BOURSOBANK/export_livret_bourso.csv (opérations livret)
  - dropbox/BOURSOBANK/export-operations-*.csv (mouvements titres horodatés)
  - dropbox/BOURSOBANK/export-positions-instantanees-*.csv (positions titres)
  - dropbox/BOURSOBANK/Portefeuille - BoursoBank.pdf (solde Espèces portefeuille)
  - dropbox/BOURSOBANK/Mes Comptes - BoursoBank.pdf (soldes tous comptes)
"""
...
1 seul par slot (1 chèque, 1 livret, 1 titres). Champ technique (onglet Comptes) :

- Numéro : colonne "accountNum" du CSV exporté (export_compte_principal.csv,
  export_livret_bourso.csv). À défaut de correspondance, le libellé du relevé
  (accountLabel) est utilisé.
...
class BbFetcher(BaseFetcher):
    ...
    def _decode_keyboard_matrix(self):
        ...
        try:
            ...
            self.logger.error(tesseract_install_hint())
            return {}

        # Configure pytesseract.tesseract_cmd avec auto-détection — le
        # subprocess `tesseract` peut être hors PATH (cas CLI Mac sans
        # /opt/local/bin chargé), auquel cas pytesseract chercherait
        # `tesseract` dans le PATH et plante.
        ...
        try:
            ...
            # Sur écran Retina (Mac), Playwright/Chromium capture le screenshot
            # à la résolution physique (devicePixelRatio = 2) mais
            # `bounding_box()` retourne des coords CSS (1×). Sans correction,
            # `crop(x, y, x+w, y+h)` cible un quart de bouton en haut-gauche
            # → l'OCR ne voit ni chiffres ni lettres (0/10 sur Mac Retina).
            viewport = self.page.viewport_size
            scale = (full_screenshot.width / viewport['width']) if viewport else 1.0
            self.logger.info(
                f"Screenshot scale {scale:.2f}× (viewport "
                f"{viewport['width'] if viewport else '?'}px → image "
                f"{full_screenshot.width}px)")
            ...
            for button in buttons:
                try:
                    ...
                    # Appliquer le scale Retina (CSS coords → pixels image)
                    if scale != 1.0:
                        box = {k: v * scale for k, v in box.items()}
            ...
            if len(no_letter_buttons) == 2:
                ...
                if d1 == '0' and d2 != '0':
                    ...
                    px1 = self._count_digit_pixels(btn1, full_screenshot, scale=scale)
                    px2 = self._count_digit_pixels(btn2, full_screenshot, scale=scale)
    ...
    def _ocr_button_letters(self, box, full_screenshot, pytesseract, Image, ImageEnhance):
        ...
        for psm in [7, 8, 13]:
            ...
            # Seuil OS-aware. Sur un vrai bouton phone keypad il y a 3-4
            # lettres (ABC, DEF, GHI, JKL, MNO, PQRS, TUV, WXYZ) ; le bouton
            # "1" n'a PAS de lettres.
            # - Mac : tesseract détecte parfois 1 lettre parasite dans la
            #   zone vide sous le "1" → seuil 2 pour éviter le faux positif
            #   (sinon "1" est classé comme "4" si la lettre parasite est
            #   un G, etc., créant un doublon).
            # - Linux : OCR plus stable historiquement (TNR pipe DEV jamais
            #   incident) → seuil 1 conservé pour ne pas régresser sur des
            #   boutons où tesseract ne lit qu'une lettre sur 3.
            min_letters = 2 if sys.platform == 'darwin' else 1
    ...
    def _count_digit_pixels(self, button, full_screenshot, scale=1.0):
        ...
        Args:
            scale: facteur d'échelle CSS → image (Retina Mac : ~2.0).
        """
        box = button.bounding_box()
        if not box:
            return 0
    ...
    def export_titres_complete(self):
        ...
        try:
            ...
            # Requête HTTP directe (cf. mouvements titres) plutôt que
            # _click_and_download : sur Mac Chromium, l'event download
            # ne se déclenche pas pour ce lien (timeout 30s observé).
            try:
                ...
                if not csv_href:
                    self.logger.warning("  Lien Exporter en CSV positions sans href")
                    self._dump_page_debug('download_fail_positions', force=True)
                else:
                    if csv_href.startswith('/'):
                        csv_href = self.base_url + csv_href
                    target_name = f"export-positions-instantanees-{datetime.now():%Y%m%d}.csv"
                    target_path = self.dropbox_dir / target_name
                    if not self._fetch_download(csv_href, target_path, 'positions'):
                        self._dump_page_debug('download_fail_positions', force=True)
            except Exception as e:
    ...
    def print_accueil_pdf(self):
        ...
        try:
            ...
            # Timeout court (5s) : non-bloquant — save_page_as_pdf produit
            # un PDF correct même sans (cf. journal 2026-05-18). Le sélecteur
            # .c-card-account n'est plus toujours visible côté BoursoBank.
            try:
                self.page.locator(".c-card-account").first.wait_for(
                    state="visible", timeout=5000
                )
            except Exception:
                self.logger.warning("Timeout chargement page accueil (non-bloquant)")
