...
class KrakenFetcher(BaseFetcher):
    ...
    def _select_csv_format(self):
        ...
        Le dropdown Format est un <input role='combobox' value='PDF'> dans
        le dialog modal, avec un <label>Format</label> sibling. On cible
        via get_by_label scopé au dialog — les sélecteurs `:near(...)`
        risquent de matcher un combobox du datePicker rendu en portail.
        """
        self.logger.debug("Sélection format CSV...")

        dialog = self.page.locator("div[role='dialog']")
        combo = dialog.get_by_label("Format", exact=True)
        if combo.count() == 0:
