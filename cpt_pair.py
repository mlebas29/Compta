...
def _build_transfer_pairs():
    ...
    try:
        ...
        _acc = {}
    pairs = {}
    for p in _acc.get('transfer_pairs', []):
        name = p.get('name')
        if name and 'source' in p and 'dest' in p:
            pairs[name] = {
                'source': p['source'],
                'dest': p['dest'],
                'max_jours_ecart': p.get('max_jours_ecart', 7),
            }
    return pairs


TRANSFER_PAIRS = _build_transfer_pairs()

# Virements internes (hub & spokes) — dérivés de config_accounts.json (zéro nom en dur)
def _build_internal_transfers():
    ...
    _sg = _acc.get('SOCGEN', {}).get('accounts', [])
    _hub = next((a['name'] for a in _sg if a.get('type_sg') == 'principal'), None)
...
INTERNAL_TRANSFERS = _build_internal_transfers()
