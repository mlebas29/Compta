...
Un PRV **Solo** sort donc du fork en **Hub local** : l'outillage (git nu `push`/`pull`, `tool_audit_git` pour status/align) fonctionne ensuite à l'identique du Hub distant. Pour poser/changer le mode ou réparer un raccourci **sans réinstaller** : `install_fix.sh [EX|PROD|DEV]`. Spec détaillée : [`Compta_tools.md`](Compta_tools.md).
...
- **DEV** édite : commit/push en **git nu, par dépôt** (`custom/` → PRV, le reste → PUB). Spec : [`Compta_tools.md`](Compta_tools.md).
