...
# tkinter optionnel : import top-level échoue sur LO Python 3.9 (sans tkinter).
# Les méthodes UI plantent alors si appelées, mais HeadlessGUI (qui hérite de
# CategoriesMixin) ne les appelle pas — il n'utilise que les workers UNO.
try:
...
class CategoriesMixin:
    ...
    def _run_budget_save(self, name, poste, alloc_pct):
        ...
        def worker():
            if HAS_UNO:
                self._save_budget_category(name, poste, alloc_pct)
            else:
                self._daemon_call('add_category', name=name, poste=poste,
                                  alloc_pct=alloc_pct)
    ...
    def _save_budget_category(self, name, poste, alloc_pct=100.0, doc=None):
        ...
        self._add_category(name, poste=poste, alloc_pct=alloc_pct / 100, doc=doc)
    ...
    def _budget_cat_delete_confirm(self, cat_name):
        ...
        if ops_count > 0:
            ...
            def on_ok():
                ...
                def worker():
                    if HAS_UNO:
                        self._delete_budget_category(cat_name, reassign_to=target)
                    else:
                        self._daemon_call('delete_category',
                                          name=cat_name, reassign_to=target)
            ...
            def worker():
                if HAS_UNO:
                    self._delete_budget_category(cat_name)
                else:
                    self._daemon_call('delete_category', name=cat_name)
    ...
    def _rename_budget_category(self, old_name, new_name, doc=None):
    ...
    def _budget_post_delete(self):
        ...
        def worker():
            if HAS_UNO:
                self._delete_budget_post(name)
            else:
                self._daemon_call('delete_poste', name=name)
    ...
    def _budget_post_dialog(self, title, name='', type_fv='', edit_name=None):
        ...
        def on_ok():
            ...
            if edit_name:
                def worker_update():
                    if HAS_UNO:
                        self._update_budget_post(edit_name, new_name, new_type)
                    else:
                        self._daemon_call('update_poste',
                                          old_name=edit_name, new_name=new_name,
                                          new_type=new_type)
                self._run_uno_operation(
                    'Modification en cours',
                    worker_update,
                ...
                def worker_save():
                    if HAS_UNO:
                        self._save_budget_post(new_name, new_type)
                    else:
                        self._daemon_call('add_poste',
                                          name=new_name,
                                          fixe=(new_type == 'Fixe'))
                self._run_uno_operation(
                    'Écriture en cours',
    ...
    def _save_budget_post(self, name, type_fv, doc=None):
    ...
    def _update_budget_post(self, old_name, new_name, new_type, doc=None):
    ...
    def _delete_budget_post(self, name, doc=None):
