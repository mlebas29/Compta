...
class _LazyFormatModules:
    """Dict-like lazy : charge cpt_format_<site> seulement à la demande.

    Évite l'import eager de tous les modules au démarrage, qui rendait une
    dep manquante (ex : pdfplumber pour un site donné) fatale pour TOUS les
    sites — y compris ceux qui n'en ont pas besoin. Le filet attrape aussi
    SystemExit (un module qui ferait sys.exit au top-level désactive juste
    son site au lieu de tuer cpt_update).

    L'API mime les méthodes utilisées en aval (keys/items/get/in/iter) pour
    que les callers existants tournent sans modification.
    """
    def __init__(self, sites):
        self._sites = list(sites)
        self._cache = {}  # site -> module | None

    def _resolve(self, site):
        if site not in self._cache:
            module_name = f'cpt_format_{site}'
            try:
                self._cache[site] = importlib.import_module(module_name)
            except ModuleNotFoundError:
                self._cache[site] = None  # Site sans formatteur
            except SystemExit as e:
                print(f"⚠ Site {site} désactivé (sys.exit au top-level) : {e}",
                      file=sys.stderr)
                self._cache[site] = None
            except (ValueError, KeyError, ImportError) as e:
                print(f"⚠ Site {site} désactivé : {e}", file=sys.stderr)
                self._cache[site] = None
    ...
    def items(self):
        for site in self._sites:
            mod = self._resolve(site)
            if mod is not None:
                yield site, mod

    def get(self, site, default=None):
        mod = self._resolve(site) if site in self._sites else None
        return mod if mod is not None else default

    def __contains__(self, site):
        return site in self._sites and self._resolve(site) is not None
...
def _load_format_modules(config):
    """Découvre les sites depuis config.ini et retourne un dict-like lazy.

    Les modules cpt_format_<site> sont importés au premier accès via la
    classe _LazyFormatModules (cf. docstring). Avant le refactor lazy,
    l'import eager faisait tomber tout cpt_update si un seul site avait
    une dep manquante au top-level.
    """
    sites = [s for s in config.sections() if s not in _CONFIG_META_SECTIONS]
    return _LazyFormatModules(sites)
...
def add_hds(filename, hds):
    ...
    Exemples:
        ...
        add_hds("@REPORT_00001_20250101_20250131.PDF", "20251120_153045")
        → "@REPORT_00001_20250101_20250131_HDS_20251120_153045.PDF"
    """
...
def parse_filename(filename):
    ...
    Exemples (ancienne convention):
        ...
        parse_filename("positions_assurance-vie-1_parsed.xlsx")
        → {'type': 'positions', 'account': 'assurance-vie-1', ...}
    ...
    # - type: operations, positions, soldes
    # - account: peut contenir des tirets (ex: assurance-vie-1)
    # - method: download, parsed
    ...
    if not match:
        # Cas spécial PDFREPORT: operations_{nom_rapport}_download.pdf
        # Ex: operations_@REPORT_00001_20250101_20250131_download.pdf
        match_report = re.match(
        ...
        if match_report:
            return {
                'type': match_report.group(1).lower(),
                'account': 'pdfreport',  # Compte virtuel pour PDFREPORT
                'method': match_report.group(3).lower(),
                'discriminant': None,
                'ext': match_report.group(4).lower()
            }
        return None
...
class ComptaUpdater:
    ...
    def restore_fallback(self):
        ...
        try:
            for hds, archive_path in archived_files:
                ...
                # archives/SOCGEN/file_HDS_xxx.csv → dropbox/SOCGEN/file.csv
                # archives/PDFREPORT/report_HDS_xxx.pdf → dropbox/PDFREPORT/report.pdf
    ...
    def archive_file(self, file_path):
        ...
        """
        # Déterminer le sous-répertoire source (PDFREPORT, SOCGEN, etc.)
    ...
    def deduplicate_files_by_basename(self, files):
        ...
        Ex: 12345678901.csv, 12345678901(1).csv, 12345678901(2).csv
        → même base "12345678901.csv" → garder le plus récent (mtime)
    ...
    def print_valorisations_summary(self):
        ...
        if self.excel.comptes_valorisations:
            ...
            for compte, valorisation in self.excel.comptes_valorisations.items():
                # Nom du compte pour l'affichage
                compte_court = compte
