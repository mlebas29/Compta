...
Usage:
    ...
    cpt.py --sites SOCGEN               # Collecte + Import uniquement SG
    cpt.py --sites SOCGEN,WISE    # Collecte + Import plusieurs sites
...
def run_script(script_name, args_list):
    ...
    try:
        # Invocation directe (pas `sys.executable script`) pour respecter le
        # shebang du script cible — sinon les scripts UNO migrés python3-uno
        # se font court-circuiter par le python3 système (cf. doctrine
        # CLAUDE_mac.md : "Subprocess GUI UNO : invocation directe").
        result = subprocess.run(
            [str(script_path)] + args_list,
            cwd=str(BASE_DIR)
        )
...
def do_push(force=False):
    ...
    result = subprocess.run(
        [str(BASE_DIR / 'tool_controles.py')],
    ...
    # Invocation directe : tool_refs.py utilise UNO, son shebang python3-uno
    # doit pouvoir s'imposer.
    result = subprocess.run(
        [str(BASE_DIR / 'tool_refs.py'), '--audit'],
        capture_output=True, text=True
    )
    ...
    result = subprocess.run(
        [sys.executable, str(BASE_DIR / 'tool_categories_audit.py'), '--lines', '500'],
        capture_output=True, text=True
    )
...
Exemples:
  ...
  %(prog)s --sites SOCGEN                # Collecte + Import uniquement SG
  %(prog)s --sites SOCGEN,WISE     # Collecte + Import plusieurs sites
...
Workflow:
  ...
  3. Cotations (cpt_fetch_quotes.py)
    ...
    # Contrôle de cohérence environnement (wrapper python3-uno, etc.)
    # Warning non bloquant : --status ou --pull n'ont pas besoin du wrapper.
    _env_ok, _env_msg = check_env()
    if not _env_ok:
        logger.warning(_env_msg)
